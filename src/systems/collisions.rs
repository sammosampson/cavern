...
pub fn collision(world: &mut SubWorld, buffer: &mut CommandBuffer) {
    let static_bodies: Vec<AABB> = <(&Position, &CollisionBox)>::query()
        ...
        .iter(world)
        .map(|(position, collision_box)| AABB::from((**position, **collision_box)))
        .collect();

    let dynamic_bodies: Vec<(Entity, AABB, Vector)> = <(Entity, &Position, &CollisionBox, &Velocity)>::query()
        .iter(world)
        .map(|(entity, position, collision_box, velocity)| (*entity, AABB::from((**position, **collision_box)), **velocity))
        .collect();
        ...
        for static_aabb in static_bodies.iter() {
            ...
            if collision_time < 1.0 {    
                ...
                if collision_normal.y != 0.0 {
                    *velocity = Vector::new(velocity.x, velocity.y * collision_time);
                }

                if collision_normal.x != 0.0 {
                    *velocity = Vector::new(velocity.x * collision_time, velocity.y);
                }

                buffer.add_component(*dynamic_entity, velocity)
            }
        }    
    }
}